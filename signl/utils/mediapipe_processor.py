...
import numpy as np
from signl.utils.one_euro_filter import OneEuroFilter
from signl.models.advanced_emotion_processor import AdvancedEmotionProcessor
import threading
...
class EmotionDetector:
    ...
    def __init__(self):
        ...
        # Maintain running stats for optional adaptive calibration
        self._feature_stats = {
            'mouth_ratio': [],
            'eye_ratio': [],
            'eyebrow_ratio': [],
    ...
    def analyze_emotion(self, face_landmarks) -> Dict[str, float]:
        ...
        # Update running stats (keep last 120 frames) for adaptive normalization
        def _update_stat(key, value):
            arr = self._feature_stats[key]
            arr.append(value)
            if len(arr) > 120:
                arr.pop(0)
        _update_stat('mouth_ratio', mouth_ratio)
        _update_stat('eye_ratio', eye_ratio)
        _update_stat('eyebrow_ratio', eyebrow_ratio)
        _update_stat('cheek_ratio', cheek_ratio)
        ...
        def _norm(key, value):
            ...
            return (value - mu) / (sigma + 1e-5)

        # Normalized variants improve stability across lighting/camera differences
        mouth_n = _norm('mouth_ratio', mouth_ratio)
        eye_n = _norm('eye_ratio', eye_ratio)
        eyebrow_n = _norm('eyebrow_ratio', eyebrow_ratio)
        cheek_n = _norm('cheek_ratio', cheek_ratio)
        ...
        if mouth_n > 0.8 and cheek_n > 0.5:
            emotions['happy'] = min(1.0, (mouth_n * 0.6 + cheek_n * 0.4))
            
        # Sad: Mouth corners down, eyebrows down
        if mouth_n < -0.6 and eyebrow_n < -0.3:
            emotions['sad'] = min(1.0, (-mouth_n * 0.55 + -eyebrow_n * 0.45))
            
        # Surprised: Eyes wide, eyebrows up
        if eye_n > 0.9 and eyebrow_n > 0.6:
            emotions['surprised'] = min(1.0, (eye_n * 0.6 + eyebrow_n * 0.4))
            
        # Angry: Eyebrows down, mouth tight or frown
        if eyebrow_n < -1.0:
            emotions['angry'] = min(1.0, (-eyebrow_n) * 0.7)
            if abs(mouth_n) < 0.3:
                emotions['angry'] = min(1.0, emotions['angry'] + 0.15)
            
        # Fearful: Eyes wide, mouth slightly open, eyebrows slightly raised
        if eye_n > 0.7 and eyebrow_n > 0.2 and abs(mouth_n) < 0.5:
            emotions['fearful'] = min(1.0, (eye_n * 0.55 + eyebrow_n * 0.45))
        
        # Disgusted: Nose wrinkle (approximated), mouth asymmetric
        if mouth_n < -0.4 and cheek_n < -0.6:
            emotions['disgusted'] = min(1.0, (-mouth_n * 0.5 + -cheek_n * 0.5))
        ...
        if max_emotion_value < 0.3:
            emotions['neutral'] = 0.65
